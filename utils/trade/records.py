...
def record_close(symbol: str, reason: str, realized_pnl: float, qty: float,
    ...
    slip_str = f" (滑点 {slippage:.4f})" if slippage > 0 else ""
    print(f"[交易流水] {symbol} {side} {tag} {realized_pnl:+.2f} USDT{fee_str}{slip_str}")


def record_open(symbol: str, side: str, qty: float, price: float,
                fee: float = 0.0, order_id: int = 0, slippage: float = 0.0,
                entry_mode: str = ""):
    record = {
        "time": datetime.now().isoformat(), "symbol": symbol, "side": side,

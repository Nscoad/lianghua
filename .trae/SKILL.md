...
一个基于 **Binance U本位合约** 的全自动量化交易系统。核心策略为**快捞**：每1分钟扫描500+币种，15分钟涨>7.3%做多、跌>7.3%做空，小仓追涨杀跌 + K线动态锁仓止盈 + 5秒持仓监控 + 滑点/资金费率追踪。趋势采集已独立为 `run_feed_collector.py` 脚本，按需运行。

**一句话**：快捞(1min监测7.3%涨跌) + 动态锁仓(K线波动算回撤) + 5秒持仓监控 + 滑点/资金费率追踪 + 定时汇总(1h/3h/6h/12h/24h)。

## 核心能力

| 能力 | 说明 |
|------|------|
| ⚡ 快捞策略 | 每1分钟监测500+币种，15min涨>7.3%做多/跌>7.3%做空，小仓追涨杀跌 |
| 🛡️ K线动态锁仓 | 利润<10%不锁，10~30%阶梯锁，≥30%固定5%价格回撤 |
| 🔄 冷却机制 | 止损后冷却10分钟，固定时间不提前解除 |
...
| 循环 | 频率 | 职责 | 代码入口 |
|------|------|------|----------|
| **快捞循环** | 每1分钟 | 监测500+币种，15min涨>7.3%做多/跌>7.3%做空 | `fast_loop()` |

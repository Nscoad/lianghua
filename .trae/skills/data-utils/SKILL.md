...
| 文件 | 说明 |
|------|------|
| `utils/db.py` | 数据库管理 — SQLite `trade_records` 建表/读写/迁移 |
| `utils/trade/records.py` | 平仓流水记录 + 开仓记录（供 `fast_trader.py` 调用）|
| `utils/trade/stats.py` | 交易统计 + 对账 + 周期统计函数 |
...
| `utils/market/kline.py` | 实时K线趋势判断（MA6/MA20） |
...
| 函数 | 说明 |
|------|------|
| `record_open(symbol, side, qty, price, fee, order_id)` | 记录开仓 |
| `record_close(symbol, reason, pnl, qty, entry, exit, side, is_partial, fee, order_id)` | 记录平仓 |
...
| `reconcile_trades(since)` | 从交易所对账补漏 |

...
这是系统的唯一交易策略——**快捞**。每1分钟监测全场币种，捕捉15分钟内涨>7.3%或跌>7.3%的异常波动币种，小仓追涨杀跌，不扛单。
...
- **冷却**：平仓后进入10分钟冷却（`_is_cooling(symbol)`），固定时间不提前解除，防止趋势未反转时反复入场
- **时间限制**：09:00~21:00 仅做多（白天震荡市偏向多），21:00~09:00 多空都做
...
3. 涨>7.3% → `try_fast_open()`：
   - 检查是否已持仓/到达上限/冷却中（`_is_cooling(symbol)`）

...
| 循环 | 频率 | 线程名 | 职责 |
|------|------|--------|------|
| **快捞循环** | 每2分钟 | Fast-Trade | `market/monitor.py:run_fast_monitor()` → 监测500+币种涨跌>6% → `trade/fast_trader.py:try_fast_open/short` 开仓 → `check_fast_position()` 持仓风控 |
...
│                    # MONITOR_INTERVAL=1800, FAST_MONITOR_INTERVAL=120
├── loops.py         # 3个循环线程函数
│   ├─ fast_loop()         快捞（2分钟）

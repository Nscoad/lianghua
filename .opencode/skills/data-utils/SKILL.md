...
- 用户要求"监控全场币种"、"检测异常涨幅"
- 用户要求"对账"、"修复交易记录"
- 系统自动调用：快捞每2分钟、趋势采集每30分钟、汇总报表每分钟检查
...
| 文件 | 主要函数 | 说明 |
|------|---------|------|
| `utils/db.py` | `init_db()`, `insert_trade_record()`, `get_trade_records()` | SQLite 建表/读写/迁移 |
...
| 文件 | 主要函数 | 说明 |
|------|---------|------|
| `utils/trade/records.py` | `record_open()`, `record_close()` | 开平仓流水记录 |
| `utils/trade/stats.py` | `calc_period_stats()`, `print_summary()`, `reconcile_trades()` | 交易统计 + 对账 |
...
| `utils/market/kline.py` | `check_kline_entry()`, `get_kline_levels()` | 实时K线趋势判断（MA6/MA20）|
...
- `reconcile_trades(since)`：交易所对账补记（启动时自动调用）

### 全场监控（market/monitor.py）

- `run_fast_monitor()`：每2分钟监测500+币种，15min涨>6%做多/跌>6%做空（含趋势过滤+冷却）

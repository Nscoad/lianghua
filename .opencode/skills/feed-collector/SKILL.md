---
name: "feed-collector"
description: 当用户要求"获取最新市场动态""爬取广场数据""看币安广场有什么消息""检查采集状态""刷新市场情绪数据""最常搜索""X.com""Twitter"时必须使用。本 skill 用 Selenium 从币安广场（关注页+主页热门）和 X.com 爬取动态并存入 JSON，供 AI 决策分析使用。
---

# 数据采集层 — 多源市场情绪采集

从币安广场（Binance Square）关注页 + 主页热门 + X.com 三个数据源爬取最新动态，供 AI 决策层分析市场情绪使用。每15分钟自动采集趋势数据。
...
| `collector/trend_collector.py` | 15分钟周期趋势采集（广场热门 + X.com） |
...
| `collect_trends(max_items_per_source=10)` | 15分钟周期采集（广场热门 + X.com） | `dict{source: {fetched, added}}` |

...
- 持仓中禁止同币种开新单
- 止损后同币种进入冷却（`_is_cooling(symbol)`，固定时间不提前解除）
- 单笔大亏损（>20 USDT）冷却延长至10分钟
...
### 快捞监测流程（每1分钟，由 `market/monitor.py` 的 `run_fast_monitor()` 触发）
...
4. **冷却检查**：同币种是否在冷却期内（`_is_cooling(symbol)`），固定时间不提前解除
...
| 冷却时间 | 止损后10分钟，固定时间不提前解除 | 大亏损(>20U)也是冷却10分钟 |
| 可同时持多币种 | 是 | 币种之间互不冲突 |
